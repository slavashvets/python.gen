...
a live PostgreSQL) into a typed Python database client. It emits exactly one
surface per generate — async (`psycopg.AsyncConnection`) by default, or sync
(`psycopg.Connection`) when `config.sync` is `True` — at the same unified
output paths either way.
...
the package-root facade module (`__init__.py`). Everything else, a
`pyproject.toml` and a `py.typed` marker, is the consumer's own hand-written
shell; the generator never touches it.
...
re-exports both from `_core` (`from ._core import JsonValue, NoRowError`)
so there is one canonical identity. The missing-single-row case
...
src/
  ...
  Structures/
    Surface.dhall            # async/sync token table, both surfaces at the same import depth (section 4)
  ...
  Interpreters/
    ...
    Query.dhall              # assemble one query: shared rowDef + one statement module for whichever surface config.sync picked
...
Text, sync : Optional Bool, onUnsupported : Optional OnUnsupported.Mode
...
below it: `{ packageName, importName, sync, onUnsupported }`, with
`packageName` falling back to the project name in kebab case, `sync` to
